...
if (isset($_POST['update'])) {
    ...
    // update user data
    $result = mysqli_query($conn, "UPDATE options SET coption='$coption' WHERE id=$id");
...
$result = mysqli_query($conn, "SELECT * FROM options WHERE id=$id");

while ($user_data = mysqli_fetch_array($result)) {
    $coption = $user_data['coption'];
}
?>
<html>
    ...
    <div class="container-fluid position-relative flex font-poppins">
        ...
        <div class="mx-auto my-10 bg-white rounded-xl shadow-lg p-10">
            ...
            <form action="" method="post" name="update_user">
                ...
                <input class="px-4 py-2 mb-8 shadow-lg rounded-lg bg-slate-200" type="text" size="80" name="coption" value="<?php echo $coption; ?>">
                <div>
                    <td><input type="hidden" name="id" value=<?php echo $_GET['id']; ?>></td>
                    <a class="float-left px-4 py-4 rounded-lg shadow-lg bg-red-500 text-white" href="jawaban.php">Kembali</a>
                    <input class="float-right px-4 py-4 rounded-lg shadow-lg bg-green-500 text-white font-bold" type="submit" name="update" value="Update">
                </div>
            </form>
        </div>
    </div>
